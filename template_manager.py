...
class RenderingError(Exception):
    def __init__(self, vlistLen, inputLen):
        self.vlistLen = vlistLen
        self.inputLen = inputLen

    def __str__(self):
        return (
            "template - RenderingError variable is "
            + self.vlistLen
            + ", but your input is "
            + self.inputLen
        )
...
def saveVariableList(template):
    # save variable list to file
    checkTemplateFolder()
    variableList = getVariableList(template)
    fileName = dir + "/" + template["templtCode"] + "_vlist.txt"
...
def RenderTemplate(template, variableList, inputList):
    # rendering template
    if len(variableList) != len(inputList):
        raise RenderingError(len(variableList), len(inputList))
    for i in range(0, len(variableList)):
        template["templtContent"] = template["templtContent"].replace(
            variableList[i], inputList[i]
        )
    return template
